...
public class Diff implements IStrategy{
    ...
    public void takeTurn(List<IPlanet> planets, IPlanetOperations planetOperations, Queue<IEvent> eventsToExecute) {
        ...
        for(Integer i : ownedPlanets) {
            ...
            for (IVisiblePlanet plan : ed) {
                if (attackPlanets.contains(plan.getId()) && idToPlanet.get(i).getPopulation() > 20) {
                    eventsToExecute.add(planetOperations.transferPeople(idToPlanet.get(i), plan, pop/(2+numFighting(planets,idToPlanet.get(i)))));
                    pop -= (pop/2);
                    sent = true;
                }
                if (help.contains(idToPlanet.get(i).getId()) && plan.getPopulation() > 20) {
                ...
                if(friendlyNeighbors(planets,idToPlanet.get(i)) && !sent) {
                    ...
                    for(IVisiblePlanet planet : lst) {
                        if(idToHelp.containsKey(planet.getId()) && idToHelp.get(planet.getId()) <
                                idToHelp.get(i)) {
    ...
    public List<IVisiblePlanet> findEdges(List<IPlanet> all, IVisiblePlanet planetOwn) {
        ...
        for(IEdge edge : edges) {
            for(IPlanet plan : all) {
                if(edge.getDestinationPlanetId() == plan.getId()) {
                    ret.add((IVisiblePlanet) plan);
                }
            }
        }
        return ret;
    ...
    public void updateHelpMap(List<IPlanet> all) {  // HashMap stored as <Planet ID, Help #>
        ...
        if(help.size() == 0) {
            ...
            int out = idToPlanet.size();
            int in = 0;

            while (idToHelp.size() < idToPlanet.size()) { // Each value
                ...
                for (Integer i : idToHelp.keySet()) {
                    ...
                    for (IVisiblePlanet plan : edge) {  // some planet sharing an edge
                        if (!idToHelp.containsKey(plan.getId())) { // new ID
                            idToHelp.put(plan.getId(), idToHelp.get(idToPlanet.get(i).getId()) + 1);
                        }

                    /* else if(idToHelp.containsKey(plan.getId()) && idToHelp.get(i) <  ) {  // ID we've seen
